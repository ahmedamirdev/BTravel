...
{
    public class GetAllContractsByCustomerId : BaseService
    {
        private readonly BaseRequest _request;

        public GetAllContractsByCustomerId(BaseRequest request)
        {
            _request = request;
        }

        public BaseResponse<IEnumerable<ContractDTO>> GetAll(string search, int userId)
        {
            ...
            var query = _request.Context.Contracts
                        ...
                        .Include(c => c.Rooms)
                        .Include(c => c.Files)
                        .Where(c => !c.IsDeleted && c.CommonUserId == userId)
